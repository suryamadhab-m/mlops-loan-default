...
def ingest_data():
    ...
    engine = create_engine('postgresql://admin:password@postgres:5432/loan_default')
...
def validate_data():
    ...
    # Manual validation without great_expectations
...
def promote_model():
    ...
    # Robust method to fetch the absolute latest version number
    versions = client.search_model_versions(f"name='{model_name}'")
    if versions:
        latest = max(int(v.version) for v in versions)
        client.transition_model_version_stage(model_name, str(latest), 'Production')
